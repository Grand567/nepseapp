...
/**
 * Validates whether a plan generated by Entry/Exit Analyzer is an authentic, actionable BUY or ACCUMULATE signal.
 * Disqualifies any plan with HOLD, WAIT, AVOID, REDUCE, NO TRADE, EXIT, STAY OUT, or institutional dumping.
 */
export function isActionableBuySignal(plan) {
  if (!plan) return false;
  const vUpper = String(plan.verdict || '').toUpperCase();
  const score = Number(plan.setupScore || plan.score || plan.compositeScore || 0);

  // 1. Mandatory disqualifications
  if (
  ...
  // 2. Must be an actionable BUY or ACCUMULATE signal from Entry/Exit Analyzer
  const hasBuyKeyword =
    vUpper.includes('BUY') ||
...
export function selectMasterPrimePick(stocks = [], priceHistories = {}, brokerDataMap = {}, options = {}) {
  ...
  const evaluatePlanWithEntryExitAnalyzer = (cand) => {
    ...
    try {
      ...
      // Must not be HOLD, WAIT, AVOID, REDUCE, NO TRADE, or institutional dumping.
  ...
  // Cache verified winner in localStorage ONLY if it is an authentic BUY/ACCUMULATE setup
  if (verifiedPrimePick && isActionableBuySignal(verifiedPrimePick)) {
    try {
      if (typeof window !== 'undefined' && window.localStorage) {
