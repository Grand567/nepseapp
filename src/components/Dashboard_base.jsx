...
import {
  ...
  BarChart2, BookOpen, Activity, Zap, Target, Calculator, BrainCircuit, Sparkles, Star
} from 'lucide-react';
import {
...
import { getProxyBase, fetchStockFundamentals } from '../utils/liveData';
...
function TradingViewChart({ history, type = 'area', height = 160 }) {
  const chartContainerRef = useRef();

  useEffect(() => {
    if (!history?.length || !chartContainerRef.current) return;
    
    // Process and sort data
    const data = history.map((h, i) => {
      // Use index fallback if date is problematic, but try to parse date
      let timeVal = i;
      if (h.dateObj) timeVal = h.dateObj.getTime() / 1000;
      else if (h.date) timeVal = new Date(h.date).getTime() / 1000;
      return {
        time: timeVal,
        value: h.close,
        open: h.open,
    ...
    // Ensure strictly ascending times for TV chart
    for (let i = 1; i < data.length; i++) {
      if (data[i].time <= data[i-1].time) data[i].time = data[i-1].time + 1;
    }
    ...
    let series;
    const isBull = data.length > 0 && data[data.length-1].close >= data[0].close;
    
    if (type === 'candle') {
      series = chart.addCandlestickSeries({
        upColor: 'rgba(16, 217, 138, 1)',
        downColor: 'rgba(245, 69, 92, 1)',
        borderVisible: false,
        wickUpColor: 'rgba(16, 217, 138, 1)',
        wickDownColor: 'rgba(245, 69, 92, 1)'
      });
      series.setData(data);
    } else {
      series = chart.addAreaSeries({
        lineColor: isBull ? '#10d98a' : '#f5455c',
        topColor: isBull ? 'rgba(16, 217, 138, 0.4)' : 'rgba(245, 69, 92, 0.4)',
...
function StockDrawer({ stock, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchLiveData = async () => {
      ...
      // Fetch fundamentals using cached utility (proxy + direct CORS fallback)
      try {
        const fundData = await fetchStockFundamentals(stock.symbol);
        if (fundData && active) {
          setLiveDetail(fundData);
        }
      } catch (e) {
        console.warn('Failed to load live fundamentals:', e.message);
      }

      // Fetch price history
      try {
        const historyRes = await fetch(`${base}/api/price-history/${stock.symbol}`);
        if (historyRes.ok) {
  ...
  // Derived stock with live fundamentals mapped
  const d = useMemo(() => {
  ...
  const activeHistory = useMemo(() => {
    return liveHistory.length > 0 ? liveHistory : history;
  }, [liveHistory, history]);
  ...
  const generateAiAnalysis = async () => {
    ...
    // Format the price history (last 15 days) for LLM analysis
    const historyText = activeHistory.slice(0, 15).map(h => `${h.date}: Rs. ${h.close || h.ltp} (Vol: ${h.volume || 0})`).join(', ');
...
Format with markdown using bold headers and clean bullet points. Do not include raw HTML.`;
    ...
    try {
      ...
      const text = data.choices?.[0]?.message?.content || "AI summary unavailable.";
      setAiResult(text);
    } catch (err) {
  ...
  // Derived analytics
  const pivot = useMemo(() => {
    const h = d.high || d.ltp * 1.03;
    const l = d.low  || d.ltp * 0.97;
    return calculatePivotPoints(h, l, d.ltp);
  }, [d]);

  const fib = useMemo(() => calculateFibonacci(d.high52w, d.low52w), [d]);

  const signal = getTechSignal(d.rsi, d.macd, d.ema20, d.ema50, d.ema200 || d.ema50 * 0.95);
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                <span className="badge badge-primary">{d.sector}</span>
                {d.pChange >= 0
                  ? <span className="badge badge-bull">▲ {Number(d.pChange || 0).toFixed(2)}%</span>
                  : <span className="badge badge-bear">▼ {Number(Math.abs(d.pChange || 0)).toFixed(2)}%</span>}
              </div>
            ...
            <div style={{ textAlign:'right' }}>
              ...
              <div style={{ fontSize:12, fontWeight:700, marginTop:2, color: d.change >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
                {d.change >= 0 ? '+' : ''}{fmt(d.change)}
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 6, justifyContent: 'flex-end' }}>
                <button onClick={() => toggleWatchlist(stock.symbol)} style={{
                  border:'1px solid var(--border)', background: isWatched ? 'rgba(255,200,0,0.15)' : 'rgba(255,255,255,0.05)',
                  borderRadius:8, padding:'4px 8px', cursor:'pointer', color: isWatched ? '#ffc107' : 'var(--text-muted)',
                  display:'flex', alignItems:'center', gap:4, fontSize:11, fontWeight:700
                }}>
                  <Star style={{width:12,height:12}} fill={isWatched ? '#ffc107' : 'none'} /> {isWatched ? 'Saved' : 'Save'}
                </button>
          ...
          {activeTab === 'overview' && (
            <div>
              ...
              <div className="chart-wrap">
                ...
                {chartMode === 'daily' ? (
                  <TradingViewChart history={activeHistory} type="area" height={220} />
                ) : (
                  <>
                    <TradingViewChart history={hourlyHistory} type="candle" height={220} />
                    <HourlyAnalysisSummary history={hourlyHistory} stock={d} />
                  </>
                )}
              </div>
              ...
              <div className="stat-grid-3" style={{marginBottom:10}}>
                ...
                <div className="stat-box">
                  ...
                  <div className="stat-box-value" style={{fontSize:12, color:'var(--bull)'}}>Rs.{fmt(d.high)}</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">Day Low</div>
                  <div className="stat-box-value" style={{fontSize:12, color:'var(--bear)'}}>Rs.{fmt(d.low)}</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">Volume</div>
                  <div className="stat-box-value" style={{fontSize:12}}>{Number(d.volume || 0).toLocaleString()}</div>
                </div>
                <div className="stat-box">
                ...
                <div className="stat-box">
                  ...
                  <div className="stat-box-value" style={{fontSize:12, color:'var(--bear)'}}>Rs.{fmt(d.low52w)}</div>
                </div>
              </div>
              ...
              <div style={{ background:signal.bg, border:`1px solid ${signal.color}30`, borderRadius:14, padding:14, marginBottom:10 }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'var(--text-muted)', marginBottom:4 }}>
                  <span>RSI({d.rsi?.toFixed(1) ?? 'N/A'})</span>
                  <span>{d.rsi < 30 ? 'Oversold' : d.rsi > 70 ? 'Overbought' : 'Neutral Zone'}</span>
                </div>
                <div className="signal-bar-wrap">
                  <div className="signal-bar" style={{
                    width: `${d.rsi ?? 50}%`,
                    background: d.rsi < 30 ? 'var(--bull)' : d.rsi > 70 ? 'var(--bear)' : 'var(--primary)'
                  }} />
                </div>
                <p style={{ fontSize:11.5, color:'rgba(255,255,255,0.72)', lineHeight:1.6, marginTop:8 }}>{signal.desc}</p>
                {/* Smart Money Concepts (SMC) detected */}
                {(() => {
                  const smcSignals = [];
                  if (d.rsi < 35 && d.pChange >= 0) smcSignals.push({ label: 'Bullish Order Block (OB)', type: 'bull' });
                  else if (d.rsi > 65 && d.pChange <= 0) smcSignals.push({ label: 'Bearish Order Block (OB)', type: 'bear' });
                  
                  if (d.volume > 100000 && Math.abs(d.pChange) > 1.5) smcSignals.push({ label: 'Fair Value Gap (FVG)', type: 'neutral' });
                  if (d.rsi < 30 && d.pChange < -2) smcSignals.push({ label: 'Liquidity Sweep', type: 'bear' });
                  if (d.rsi > 70 && d.pChange > 2) smcSignals.push({ label: 'Break of Structure (BOS)', type: 'bull' });
                  
                  if (smcSignals.length === 0) return null;
                  return (
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
                      {smcSignals.map((s, idx) => (
                        <span key={idx} style={{ 
                          fontSize: 9.5, fontWeight: 800, padding: '3px 8px', borderRadius: 4, textTransform: 'uppercase',
                          background: s.type === 'bull' ? 'rgba(16, 217, 138, 0.15)' : s.type === 'bear' ? 'rgba(245, 69, 92, 0.15)' : 'rgba(255, 255, 255, 0.1)',
                          color: s.type === 'bull' ? 'var(--bull)' : s.type === 'bear' ? 'var(--bear)' : 'var(--text-secondary)',
                          border: `1px solid ${s.type === 'bull' ? 'rgba(16, 217, 138, 0.3)' : s.type === 'bear' ? 'rgba(245, 69, 92, 0.3)' : 'rgba(255, 255, 255, 0.2)'}`
                        }}>
                          {s.label}
                        </span>
                      ))}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginTop:10 }}>
                  {[
                    ['EMA20', `Rs.${fmt(d.ema20 || d.ltp)}`],
                    ['EMA50', `Rs.${fmt(d.ema50 || d.ltp)}`],
                    ['MACD Line', d.macd?.line?.toFixed(3) ?? '0.000'],
                    ['MACD Signal', d.macd?.signal?.toFixed(3) ?? '0.000'],
                  ].map(([l,v]) => (
              ...
              <div className="stat-grid-3">
                <div className="stat-box">
                  ...
                  <div className="stat-box-value">{d.eps > 0 ? d.eps : 'N/A'}</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">P/E</div>
                  <div className="stat-box-value">{d.pe > 0 ? d.pe : 'N/A'}</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">P/B</div>
                  <div className="stat-box-value">{d.pb?.toFixed(2) || 'N/A'}</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">Book Val</div>
                  <div className="stat-box-value" style={{fontSize:12}}>Rs.{d.bookValue}</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">Div Yield</div>
                  <div className="stat-box-value">{d.divYield}%</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-label">ROE</div>
                  <div className="stat-box-value">{d.roe}%</div>
                </div>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{
                ...
                <div style={{fontSize:28, fontWeight:900, letterSpacing:'-0.04em', color:'var(--text-primary)', fontFamily:'var(--font-mono)'}}>
                  Rs.{fmtM(d.marketCap || d.ltp * d.listedShares)}
                </div>
                <div style={{fontSize:11, color:'var(--text-muted)', marginTop:4}}>
                  {fmt(d.listedShares)}M shares listed · Paid-up Capital Rs.{fmt(d.paidUpCapital)}M
                </div>
              </div>
              ...
              <div className="stat-grid-2" style={{marginBottom:12}}>
                {[
                  ['EPS (Earnings/Share)', `Rs. ${d.eps > 0 ? d.eps : 'N/A'}`],
                  ['P/E Ratio', d.pe > 0 ? `${d.pe}x` : 'N/A'],
                  ['P/B Ratio', d.pb ? `${d.pb.toFixed(2)}x` : 'N/A'],
                  ['Book Value/Share', `Rs. ${d.bookValue}`],
                  ['Return on Equity', `${d.roe}%`],
                  ['Dividend Yield', `${d.divYield}%`],
                  ['Outstanding Shares', `${d.listedShares}M`],
                  ['120D Avg Price', `Rs. ${d.avg120}`],
                ].map(([l,v]) => (
              ...
              <div style={{background:'rgba(255,255,255,0.025)', border:'1px solid var(--border)', borderRadius:12, padding:14, marginBottom:12}}>
                ...
                <div className="stat-grid-2">
                  <div>
                    ...
                    <div style={{fontSize:20, fontWeight:900, color: d.bonusShare > 0 ? 'var(--bull)' : 'var(--text-muted)', fontFamily:'var(--font-mono)'}}>
                      {d.bonusShare > 0 ? `${d.bonusShare}%` : '—'}
                    </div>
                  </div>
                  <div>
                    <div style={{fontSize:10, color:'var(--text-muted)', fontWeight:700, marginBottom:4}}>CASH DIVIDEND</div>
                    <div style={{fontSize:20, fontWeight:900, color: d.cashDiv > 0 ? 'var(--accent-cyan)' : 'var(--text-muted)', fontFamily:'var(--font-mono)'}}>
                      {d.cashDiv > 0 ? `${d.cashDiv}%` : '—'}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{background:'rgba(255,255,255,0.025)', border:'1px solid var(--border)', borderRadius:12, padding:14}}>
                ...
                <div style={{position:'relative', height:8, background:'rgba(255,255,255,0.08)', borderRadius:8, margin:'0 0 8px'}}>
                  {(() => {
                    const pct = ((d.ltp - d.low52w) / (d.high52w - d.low52w)) * 100;
                    return (
                      <>
                ...
                <div style={{display:'flex', justifyContent:'space-between', fontSize:11, fontWeight:700}}>
                  <span style={{color:'var(--bear)'}}>Low Rs.{fmt(d.low52w)}</span>
                  <span style={{color:'var(--text-muted)', fontSize:10}}>Current Rs.{fmt(d.ltp)}</span>
                  <span style={{color:'var(--bull)'}}>High Rs.{fmt(d.high52w)}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'technicals' && (
            <div>
              ...
              <div style={{marginBottom:12}}>
                ...
                <div className="stat-grid-3">
                  {[
                    ['EMA 20', d.ema20, 'Short-term'],
                    ['EMA 50', d.ema50, 'Mid-term'],
                    ['EMA 200', d.ema200 || (d.ema50 * 0.95), 'Long-term'],
                  ].map(([label, val, sub]) => {
                    const aboveLtp = d.ltp >= val;
                    return (
              ...
              <div style={{background:'rgba(255,255,255,0.025)', border:'1px solid var(--border)', borderRadius:12, padding:14, marginBottom:12}}>
                ...
                <p style={{fontSize:10, color:'var(--text-muted)', marginTop:8, lineHeight:1.5}}>
                  Pivot calculated from today's High ({fmt(d.high)}), Low ({fmt(d.low)}), Close ({fmt(d.ltp)}).
                </p>
              </div>
              ...
              <div style={{background:'rgba(255,255,255,0.025)', border:'1px solid var(--border)', borderRadius:12, padding:14}}>
                ...
                {[
                  ...
                  const isNear = Math.abs(d.ltp - f.val) / d.ltp < 0.015;
                  return (
                      ...
                      <>
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',fontSize:11}}>
                          ...
                          <span style={{fontFamily:'var(--font-mono)',fontWeight:800,color:calcResult.roi>=0?'var(--bull)':'var(--bear)'}}>
                            {calcResult.roi >= 0 ? '+' : ''}{Number(calcResult.roi || 0).toFixed(2)}%
                          </span>
                        </div>
                      </>
                    )}
