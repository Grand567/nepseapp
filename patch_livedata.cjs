...
const target = `export const fetchStockFundamentals = async (symbol) => {
  try {
...
const replacement = `export const fetchStockFundamentals = async (symbol) => {
  try {
    const isNative = Capacitor.isNativePlatform();
    // Switched to Merolagani to bypass ShareSansar broken quarterly reports API
    const rawUrl = \`https://merolagani.com/CompanyDetail.aspx?symbol=\${symbol.toUpperCase()}\`;
    const targetUrl = isNative ? rawUrl : \`\${CORS_PROXY}\${encodeURIComponent(rawUrl)}\`;
...
console.log('Patched liveData.js for Merolagani');
